	...
	public List<CodigoDTO> recuperarCodigos(String sistema, String tipo) {
		...
		while (rowIterator.hasNext()) {
			...
			if (contador > 1) {// OJO
					...
					if (cell.getRowIndex() > 1) {
						...
						if (indiceColumna == 0) {
							if (cell.getCellType() == Cell.CELL_TYPE_STRING || cell.getCellType() == Cell.CELL_TYPE_FORMULA) {
								...
								codigo.setCodigo(Integer.toString((int) valorNumerico));
							} else {
				...
				/*
				 * 
				 * String codigoA = ""; String sistemaA = ""; String tipoA = "";
				 * String codigoB = ""; String sistemaB = ""; String tipoB = "";
				 * while (cellIterator.hasNext()) {
				 * 
				 * Cell cell = cellIterator.next(); if(cell.getRowIndex()>0){
				 * int indiceColumna = cell.getColumnIndex(); if(indiceColumna
				 * == configuracion.getUbicacionCodigoA()){ switch
				 * (cell.getCellType()) { case Cell.CELL_TYPE_NUMERIC: codigoA =
				 * (new Integer((new
				 * Double(cell.getNumericCellValue())).intValue())).toString();
				 * codigoA =
				 * obtenerCorrelacionCPTDeterminacion(configuracionAMedida,
				 * codigoA); break; case Cell.CELL_TYPE_STRING: codigoA =
				 * cell.getStringCellValue(); codigoA =
				 * obtenerCorrelacionCPTDeterminacion(configuracionAMedida,
				 * codigoA); break; } //break; }
				 * 
				 * 
				 * 
				 * // Evitamos la primera fila (con los titulos) if(contador>0){
				 * codigo.setCodigo(row.getCell(0).getStringCellValue());
				 * codigo.setDescripcion(row.getCell(1).getStringCellValue());
				 * codigo.setSistema(sistema); codigo.setTipo(tipo); }
				 */
					...
					if (cell.getRowIndex() > 1) {
						...
						if (indiceColumna == 0) {
							if (cell.getCellType() == Cell.CELL_TYPE_STRING) {
								...
								double valorNumerico = cell.getNumericCellValue();
								correlacion.setCodigoA(Integer.toString((int) valorNumerico));
							} else {
							...
							if (cell.getCellType() == Cell.CELL_TYPE_STRING) {
								...
								double valorNumerico = cell.getNumericCellValue();
								correlacion.setCodigoB(Integer.toString((int) valorNumerico));
							} else {
